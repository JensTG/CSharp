...
namespace ProjectEuler
{
    ...
    class GridProduct
    {
        static void Main()
        {
            ...
            // Vertical:
            for (int r = 0; r < grid.Count - 3; r++)
            {
                for (int c = 0; c < grid[0].Count; c++)
                {
                    ulong product = (ulong)(grid[r][c] * grid[r + 1][c] * grid[r + 2][c] * grid[r + 3][c]);
            ...
            {
                for (int c = 0; c < grid[0].Count - 3; c++)
                {
                    ulong product = (ulong)(grid[r][c] * grid[r][c + 1] * grid[r][c + 2] * grid[r][c + 3]);
                    largest = product > largest ? product : largest;
                }
            }

            // Diagonal /:
            for (int r = 4; r < grid.Count; r++)
            {
                for (int c = 0; c < grid[0].Count - 3; c++)
                {
                    ulong product = (ulong)(grid[r][c] * grid[r - 1][c + 1] * grid[r - 2][c + 2] * grid[r - 3][c + 3]);
                    largest = product > largest ? product : largest;
                }
            }
